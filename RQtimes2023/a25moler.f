...
C     REAL EPS,P0,X(N),S(N),T(N),U(N),V(N),W(N),Y(N),RNORM
      ...
      DOUBLE PRECISION EPS,P0,RNORM,VNORM,RNV, SCALM, GG
...
C  I/O CHANNELS
 ...
 900  FORMAT(I6, F10.0)
      LIMIT=1000*N
C      WRITE(NOUT,950)N,LIMIT
 950  FORMAT(' MOLER MATRIX ORDER',I5,'  LIMIT=',I7)
      IF(N.LE.0)STOP
C      IF(SCALM.LT.0.0) WRITE(NOUT,953)
...
C APPROX
      ...
      DO 10 I=1,N
        X(I)=RNORM
  10  CONTINUE
C      write(NOUT,*) " About to call A25RQM"
      CALL A25RQM(N,X,EPS,KPR,S,T,U,V,W,Y,P0,NOUT,APR,BPR,SCALM,GG)
      P0=P0*SCALM
      WRITE(NOUT,951)N,KPR,P0,GG
 951  FORMAT(' ',I6, I6, 1PE16.8, 1PE16.8)
...
C  UNITM MATRIX * X  INTO V
 ...
 100  CONTINUE
      RETURN
      END
      SUBROUTINE APR(N,X,V,SCALM)
      integer n, i, j
      double precision x(n), V(n), sum, SCALM
...
C use negative to get largest ev
 20   continue
      ...
      SUBROUTINE A25RQM(N,X,EPS,KPR,Y,Z,T,G,A,B,P0,IPR,APR,BPR,SM,GG)
C  STEP 0
...
C  STEP 4
      P0=XAX/XBX
      IF(P0.GE.PA)RETURN
C      IF(IPR.GT.0)WRITE(IPR,963)KPR,P0
...
C  STEP 6
  ...
  65  CONTINUE
C  STEP 7
C      IF(IPR.GT.0)WRITE(IPR,964)GG

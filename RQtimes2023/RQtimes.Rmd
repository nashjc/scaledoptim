...
```{r setup, echo=FALSE}
mbt<-25 # default to 2 repetitions in microbenchmark while sorting out text
...
axtym<-data.frame(n=ptable[,1], ax=ptable[,2], axr=ptable[,3],  axx=ptable[,4],
                  axxr=ptable[,5],  axftn=ptable[,6], axftnr=ptable[,7], 
                  aximp=ptable[,8], aximpr=ptable[,9], 
                  axmfast=ptable[,10], axmfastr=ptable[,11])
print(axtym)
```

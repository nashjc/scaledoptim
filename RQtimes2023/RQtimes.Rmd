...
```{r setup, echo=FALSE}
mbt<-2 # default to 10 repetitions in microbenchmark
```
...
created as almost perfect quadratic models in `n`. However,
...
for (ni in 1:nmax){
  ...
  stable[[ni, 2]]<-mean(tspg$time)
  stable[[ni, 3]]<-mean(tbld$time)
...
for (ni in 1:nmax){
  ...
  x<-runif(n) # generate a vector 
  AA<-molerfast(n) # make sure defined
  tcgu<-microbenchmark(arcgu<-Rcgminu(x, fn=nobj, gr=ngrobj, AA=-AA), times=mbt)
  ctable[[ni,1]]<-n
  ctable[[ni,2]]<-tcgu
}
...
t2000g<-microbenchmark(ag<-geradin(x, naxftnc, ident, AA=1, BB=1, control=list(trace=FALSE)), times=mbt)
...
- The \code{crossprod()} function is very efficient.
...
- The \code{eigen()} routine is a highly effective tool for
